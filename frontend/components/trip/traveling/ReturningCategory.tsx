...
interface ReturningCategoryProps {
    tripId: number
    isOpen: boolean
    toggleOpen: () => void
...
export function ReturningCategory({ tripId, isOpen, toggleOpen, category }: ReturningCategoryProps) {
    return (
        <Collapsible
            open={isOpen}
                    ...
                    <div className="px-3 pb-3 space-y-2">
                        {category.items.length === 0 ? (
                            ...
                            category.items.map((item) => (
                                <div key={item.id}>
                                    <ReturningItem tripId={tripId} item={item} />
                                </div>
                            ))
                        )}
                    </div>
