...
interface PurchasedCategoryProps {
    tripId: number
    isOpen: boolean
    toggleOpen: () => void
...
export function PurchasedCategory({ tripId, isOpen, toggleOpen, category, onAddItem, onMoveItem }: PurchasedCategoryProps) {
                    ...
                    <div className="px-3 pb-3 space-y-2">
                        {category.items.length === 0 ? (
                            ...
                            category.items.map((item) => (
                                <div key={item.id}>
                                    <PurchasedItem tripId={tripId} item={item} onMove={() => onMoveItem(item)} />
                                </div>
                            ))
                        )}
                    </div>
